...
class WinConditionLogicTest {
	...
	void testHighCard() {
		...
		List <Card> cards = Arrays.asList(new Card[] {card1,card2,card3,card4,card5,card6,card7});
		WinningCombination result = WinConditionLogic.findWinningCombination(cards);
	 ...
	 * Also tests that if we have a pair of a card from the straight, we still get a straight
	 */
